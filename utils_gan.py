...
def save_data(data, depth, fake_center, gt, path ,idx):
    for index in range(9):
        ...
        train_img.save(os.path.join(path, '%d_%d.png' % (index,idx)))
...
def validation(val_dataloader, net, epoch, best):
    """
    ...
    criterion.cuda()
    with torch.no_grad():
        for i, (input_h, center, gt, conditionmat, condition, depth) in enumerate(val_dataloader):
            ssim_img = []
            ...
            if best == True:
                ...
                save_data(pred_image, fake_depth, fake_center, gt[:,:,256:512,256:512], out_save_path ,epoch)
